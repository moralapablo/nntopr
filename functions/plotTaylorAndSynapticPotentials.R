################################
## Function that allows to take an example generated with performAutomaticExample
## and plot the distribution of test synaptic potentials 
## (sum of input values * weights) at all neurons together with the Taylor expansion
## used in the activation functions.
## It also plots some error lines given a tolerance parameter tol.
## Author: Pablo Morala
###############################

plotTaylorAndSynapticPotentials <- function(example, fun, x, tol, q_taylor, title) {
  test <- example$test
  ...
  plot.density <- ggplot(df.density) +
    aes(x = x) +
  ...
  plot.Taylor <- ggplot() +
    geom_line(data = df.plot, aes(x, yf)) +
